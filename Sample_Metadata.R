...
MCGL2 <- read_excel("Z:/filepath.../MCGL Database/Working/MCGL Sample Database (11-00001 to 17-15647).xlsx", 
                    guess_max = 1000000) %>% 
...
MCGL3 <- read_excel("Z:/filepath.../MCGL Database/Working/MCGL Sample Database (18-00001 to 19-28271).xlsx",
                    guess_max = 1000000) %>% 
  select(SampleID, PlateID, ProjectID, CommonName, YearCollected, WaterbodyName, State, County, Collectors, Latitude, Longitude, Comments)

MCGL4 <- read_excel("Z:/filepath.../MCGL Database/Working/MCGL Sample Database (21-00001 to 22-19350).xlsx",
                    guess_max = 1000000) %>% 
  select(SampleID, PlateID, ProjectID, CommonName, YearCollected, WaterbodyName, State, County, Collectors, Latitude, Longitude, Comments) %>% 
...
# HUC 12 data
HUC12_WBICs <- read_delim("X:/filepath.../Statewide BKT info/HUC12_WBIC.txt") %>% 
...
Stocking_data <- read_excel("X:/filepath.../Statewide BKT info/WI_BKT_Stocking&CPUE.xlsx", 
                            sheet = "Stocking data") %>% 
...
write_csv(Stocking_data, "X:/filepath.../Stocking_histories_2205.csv")

#### Create reusable CPE data CSV ####
CPE_data <- read_excel("X:/filepath.../Statewide BKT info/WI_BKT_Stocking&CPUE.xlsx", 
                       sheet = ">200mile") %>% 
  select(WBIC, Survey_Year, Hours, N_Fish, CPE_Hour) %>% 
  rename(n_caught = N_Fish) %>% 
  filter(WBIC %in% Samples_2205$WBIC)

write_csv(CPE_data, "X:/filepath.../CPE_data_2205.csv")
