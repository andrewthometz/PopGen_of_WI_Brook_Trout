#########################################################################################
#### Produce a clean table to display measures of genetic diversity using R markdown ####
#########################################################################################
...
#### Estimate genetic diversity measures ####
library(hierfstat)
...
#### Bring in hatchery introgression data ####
hatch_int <- read_delim("X:/filepath.../Hatchery_introgression/Hatchery_ID_2205.csv") %>% 
  select(WaterbodyName, Ave_hatchery_ID) %>% 
  mutate(Ave_hatchery_ID = round(Ave_hatchery_ID*100, 2))

#### Bring in effective population size (Ne) data ####
Ne_output <- read_delim("X:/filepath.../Genetic_diversity/Ne/Ne_All_63pops_LD_CleanedUp.txt") 
...
# Write as CSV
write.csv(final_table, "X:/filepath.../Genetic_diversity/Diversity_2205.csv",
          row.names = FALSE)
...
#### Produce and customize final table ####
final_table %>% 
